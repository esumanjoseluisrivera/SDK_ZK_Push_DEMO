...
{
    /// <summary>Save ServerLog To File
    /// </summary>
        ...
        protected static string LogFile
        {
            get
            {
                if (Tools.GetDateTimeNow().Hour != m_lastDate.Hour || string.IsNullOrEmpty(m_logFile))
                {
                    ...
                    System.IO.DirectoryInfo dir = new DirectoryInfo(Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\ServerLog");
                    if (!dir.Exists)
                    {
                        dir.Create();
                    }
                    ...
                    m_logFile = dir.FullName + "\\ServerLog" + m_lastDate.Hour.ToString("00") + ".txt";
